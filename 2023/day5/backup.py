...
def recur(seed, matrix, map):
    startlist = [int(x[0]) for x in matrix[map]]
    seedstart = seed[0]
    seedend = seed[1]
    offsets = []
    startlist.append(startlist[-1]+int(matrix[map][-1][2]))
    #finding indecies for seedstart and seedend
    for i in range(len(startlist)):
        if seedstart < startlist[i]:
            sind = i
            break
        elif seedstart >= startlist[-1]:
            sind = len(startlist)
            break
    ...
    if sind == eind:
        if sind == 0 or sind == len(startlist):
            ...
            offsets.append(int(matrix[map][sind-1][1]) - int(matrix[map][sind-1][0]))
    else:
        if sind == 0:
            offsets.append(0)
        for p in range(max(sind,1),min(len(startlist),eind+1),1):
            offsets.append(int(matrix[map][p-1][1]) - int(matrix[map][p-1][0]))
    ...
    if len(offsets) == 1:
        ...
        if offsets[-1] == 0:
            offsets.pop(-1)
            inter_seed.append(startlist[-1]+int(matrix[map][-1][2]))
            inter_seed.append(seedend)
            seedlist.append(inter_seed)
            inter_seed =[]
            seedend = startlist[-1]+int(matrix[map][-1][2])-1
